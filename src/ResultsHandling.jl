...
"""
    AssignResultsFinal(Results::Dict, 
                  Con::Array{Float64,3}, 
                  Stat::Array{Float64,3}, 
                  Con_dist::Array{Float64,3}, 
                  Stat_dist::Array{Float64,3}, 
                  ...
                  dHam::Array{Float64,3}, 
                  dHam_dist::Array{Float64,3}, 
                  SearchDir::Array{Float64,3}, 
                  SearchDir_dist::Array{Float64,3}, 
                  Para::Dict)

Assign the control and state variables to the Results dictionary
"""
function AssignResultsFinal(Results::Dict, Con::Array{Float64,3}, Stat::Array{Float64,3}, Con_dist::Array{Float64,3}, Stat_dist::Array{Float64,3}, Stat_agg::Array{Float64,3}, 
                                           CoStat::Array{Float64,3}, CoStat_dist::Array{Float64,3}, dHam::Array{Float64,3}, dHam_dist::Array{Float64,3}, SearchDir::Array{Float64,3}, SearchDir_dist::Array{Float64,3}, Para::Dict)
    ...
    Results["dHam"] = dHam
    Results["dHam_dist"] = dHam_dist
    Results["SearchDir"] = SearchDir
    Results["SearchDir_dist"] = SearchDir_dist
    Results["Para"] = Para
    Results["ObjectiveValue"] = ObjectValue(Con,Stat,Con_dist,Stat_dist,Stat_agg,Para)
        ...
        for jj = ii:Para["nTime"]
            for kk = 1:Para["nCon_dist"]
                ...
                elseif Con_dist[ii,jj,kk] >= Para["Con_distMax"](Para["tmesh"][jj])[kk]
                    ...
                    dHam_dist[ii,jji,kk] = min(dHam_dist[ii,jj,kk],0)
                end
            end
        end
    end 
...
- sizeX, sizeY -> dimensions of the plots. Default = 600x400
"""
function PlotResults(Results2::Dict;SavePlot=false,Display=true,PlotFirstStage=true,sizeX=600,sizeY=400)
    ...
    if ("SearchDir" in keys(Results))
        SearchDir = Results["SearchDir"]
        SearchDir_dist = Results["SearchDir_dist"]
    else
        SearchDir = 0.0 * dHam
        SearchDir_dist = 0.0 * dHam_dist
    end
    Para = Results["Para"]

    # Adjust Gradients and SearchDirections to 0 if the solution is on the boundary
    for ii = 1:Para["nTime"]
        for kk = 1:Para["nCon"]
            if Con[1,ii,kk] <= Para["ConMin"](Para["tmesh"][ii])[kk]
                SearchDir[1,ii,kk] = max(SearchDir[1,ii,kk],0)
...
function SaveResults(Results::Dict,filepath)
    ...
    serialize(string(filepath,".jls"), Results)
end
...
function LoadResults(filepath)
    ...
    Results = deserialize(string(filepath,".jls"))
    return Results
end