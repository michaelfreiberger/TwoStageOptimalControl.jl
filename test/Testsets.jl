...
@testset "Capital accumulation" begin
    ...
    ResultsBenchmark = LoadResults("test/CapitalAccumulationBenchmark")
    ...
    @test maxControlDifference < 1e-6
    @test maxStateDifference < 1e-6
...
@testset "Test model" begin
    ...
    Results = Dict()
    Results["Con"] = 2.0*ones(1,100,1)
    Results["Con_dist"] = 2.0*ones(100,100,1)
